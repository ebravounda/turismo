...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'reservar') {
        ...
        echo json_encode(['success' => true, 'reserva' => $nuevaReserva, 'mensaje' => 'Reserva confirmada exitosamente']);
        exit;
    }
...
<head>
  ...
  <style>
    .reservas-hero { background: #1a1a1a; padding: 80px 0 40px; text-align: center; color: #fff; }
    .reservas-hero h2 { font-size: 2.6rem; font-weight: 300; letter-spacing: 3px; text-transform: uppercase; margin-bottom: 10px; }
    .reservas-hero p { color: #aaa; font-size: 1rem; letter-spacing: 1px; }
    .section-precios { background: #f7f7f7; padding: 60px 0; }
    .section-precios h3 { text-align: center; text-transform: uppercase; letter-spacing: 3px; font-weight: 300; color: #1a1a1a; margin-bottom: 40px; font-size: 1.5rem; }
    .precio-card { background: #fff; border: 1px solid #e0e0e0; border-radius: 4px; padding: 35px 25px; text-align: center; transition: all 0.3s ease; height: 100%; }
    .precio-card:hover { border-color: #1a1a1a; box-shadow: 0 6px 30px rgba(0,0,0,0.12); transform: translateY(-4px); }
    .precio-card.destacado { background: #1a1a1a; color: #fff; border-color: #1a1a1a; }
    .precio-card.destacado .precio-valor { color: #fff; }
    .precio-card.destacado .precio-label { color: #ccc; }
    .precio-card.destacado .precio-descripcion { color: #aaa; }
    .precio-icon { font-size: 2rem; margin-bottom: 15px; display: block; }
    .precio-nombre { font-size: 0.85rem; text-transform: uppercase; letter-spacing: 2px; color: #888; margin-bottom: 15px; }
    .precio-card.destacado .precio-nombre { color: #ccc; }
    .precio-valor { font-size: 2.4rem; font-weight: 700; color: #1a1a1a; line-height: 1; margin-bottom: 5px; }
    .precio-label { font-size: 0.75rem; color: #999; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 15px; }
    .precio-descripcion { font-size: 0.85rem; color: #666; border-top: 1px solid #eee; padding-top: 15px; margin-top: 10px; }
    .precio-card.destacado .precio-descripcion { border-color: #333; }
    .section-reserva { background: #fff; padding: 60px 0 80px; }
    .section-reserva h3 { text-align: center; text-transform: uppercase; letter-spacing: 3px; font-weight: 300; color: #1a1a1a; margin-bottom: 10px; font-size: 1.5rem; }
    .section-reserva .subtitulo { text-align: center; color: #888; margin-bottom: 40px; font-size: 0.9rem; letter-spacing: 1px; }
    .steps-nav { display: flex; justify-content: center; align-items: center; margin-bottom: 40px; gap: 0; }
    .step-indicator { display: flex; flex-direction: column; align-items: center; flex: 1; max-width: 160px; }
    .step-circle { width: 40px; height: 40px; border-radius: 50%; background: #e0e0e0; color: #999; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 0.9rem; margin-bottom: 6px; transition: all 0.3s; }
    .step-circle.active { background: #1a1a1a; color: #fff; }
    .step-circle.done { background: #4a4a4a; color: #fff; }
    .step-label { font-size: 0.7rem; text-transform: uppercase; letter-spacing: 1px; color: #999; text-align: center; }
    .step-label.active { color: #1a1a1a; font-weight: 700; }
    .step-line { flex: 1; height: 1px; background: #e0e0e0; max-width: 80px; margin-bottom: 26px; }
    .paso { display: none; animation: fadeInUp 0.3s ease; }
    .paso.activo { display: block; }
    @keyframes fadeInUp { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
    .fecha-selector { max-width: 480px; margin: 0 auto; text-align: center; }
    .fecha-selector label { display: block; font-size: 0.8rem; text-transform: uppercase; letter-spacing: 2px; color: #555; margin-bottom: 10px; }
    .fecha-selector input[type="date"] { width: 100%; padding: 14px 18px; border: 1px solid #ddd; border-radius: 2px; font-size: 1rem; margin-bottom: 20px; outline: none; transition: border 0.2s; font-family: inherit; }
    .fecha-selector input[type="date"]:focus { border-color: #1a1a1a; }
    .slots-header { text-align: center; margin-bottom: 25px; font-size: 0.85rem; color: #666; }
    .slots-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(130px, 1fr)); gap: 12px; max-width: 700px; margin: 0 auto 30px; }
    .slot-btn { border: 1px solid #ddd; border-radius: 3px; padding: 16px 10px; text-align: center; cursor: pointer; transition: all 0.2s; background: #fff; }
    .slot-btn:hover:not(.slot-lleno) { border-color: #1a1a1a; background: #f5f5f5; }
    .slot-btn.slot-lleno { opacity: 0.45; cursor: not-allowed; background: #f9f9f9; }
    .slot-btn.slot-seleccionado { background: #1a1a1a; border-color: #1a1a1a; color: #fff; }
    .slot-hora { font-size: 1.1rem; font-weight: 700; display: block; margin-bottom: 5px; }
    .slot-cupos { font-size: 0.7rem; text-transform: uppercase; letter-spacing: 1px; }
    ...
    .reserva-form { max-width: 560px; margin: 0 auto; }
    .resumen-seleccion { background: #f7f7f7; border-left: 3px solid #1a1a1a; padding: 16px 20px; margin-bottom: 28px; border-radius: 0 3px 3px 0; }
    .resumen-seleccion p { margin: 0; font-size: 0.88rem; color: #444; line-height: 1.8; }
    .resumen-seleccion strong { color: #1a1a1a; }
    .form-group-kp label { display: block; font-size: 0.75rem; text-transform: uppercase; letter-spacing: 1.5px; color: #555; margin-bottom: 6px; }
    .form-group-kp { margin-bottom: 20px; }
    .form-group-kp input, .form-group-kp select { width: 100%; padding: 12px 16px; border: 1px solid #ddd; border-radius: 2px; font-size: 0.95rem; font-family: inherit; outline: none; transition: border 0.2s; background: #fff; }
    .form-group-kp input:focus, .form-group-kp select:focus { border-color: #1a1a1a; }
    .precio-indicador { background: #1a1a1a; color: #fff; padding: 14px 20px; border-radius: 2px; text-align: center; margin-bottom: 24px; font-size: 0.85rem; letter-spacing: 1px; }
    .precio-indicador span { font-size: 1.3rem; font-weight: 700; display: block; margin-top: 4px; }
    .btn-kp { display: inline-block; padding: 14px 36px; font-size: 0.8rem; text-transform: uppercase; letter-spacing: 2px; font-weight: 700; border: none; cursor: pointer; transition: all 0.25s; border-radius: 2px; font-family: inherit; }
    .btn-kp-primary { background: #1a1a1a; color: #fff; }
    .btn-kp-primary:hover { background: #333; }
    .btn-kp-outline { background: transparent; color: #1a1a1a; border: 1px solid #1a1a1a; }
    .btn-kp-outline:hover { background: #1a1a1a; color: #fff; }
    .confirmacion-box { max-width: 520px; margin: 0 auto; text-align: center; }
    .confirmacion-icon { width: 70px; height: 70px; background: #1a1a1a; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 25px; font-size: 2rem; }
    .confirmacion-box h4 { font-size: 1.4rem; text-transform: uppercase; letter-spacing: 2px; font-weight: 300; margin-bottom: 15px; }
    .detalle-reserva { background: #f7f7f7; padding: 20px 25px; border-radius: 3px; text-align: left; margin: 20px 0 30px; font-size: 0.88rem; }
    .detalle-reserva dl { margin: 0; }
    .detalle-reserva dt { font-size: 0.72rem; text-transform: uppercase; letter-spacing: 1px; color: #888; margin-top: 12px; }
    .detalle-reserva dt:first-child { margin-top: 0; }
    .detalle-reserva dd { color: #1a1a1a; font-weight: 600; margin: 0; }
    .alert-kp { padding: 12px 18px; border-radius: 2px; font-size: 0.88rem; margin-bottom: 18px; display: none; }
    .alert-kp.error { background: #fdecea; color: #c62828; border-left: 3px solid #c62828; }
    .alert-kp.visible { display: block; }
    .loading-spinner { display: none; text-align: center; padding: 30px; color: #888; }
    .loading-spinner.visible { display: block; }
    .section-divider { width: 50px; height: 2px; background: #1a1a1a; margin: 0 auto 20px; }
  </style>
</head>
<body>
<div class="preloader">
        ...
        <div class="rd-navbar-main-outer">
          <div class="rd-navbar-main">
            <div class="rd-navbar-panel">
              ...
              <div class="rd-navbar-brand"><a class="brand" href="index.php"><img src="images/logo-klara.png" alt="Klara Pilates" style="height:50px;width:auto"/></a></div>
            ...
            <div class="rd-navbar-main-element">
              <div class="rd-navbar-nav-wrap">
                <ul class="rd-navbar-nav">
                  ...
                  <li class="rd-nav-item"><a class="rd-nav-link" href="index.php#services">Clases</a></li>
                  <li class="rd-nav-item"><a class="rd-nav-link" href="index.php#gallery">Galería</a></li>
  ...
  <section class="section-reserva">
    <div class="container">
      ...
      <div class="paso" id="paso-2">
        <div class="slots-header"><span id="slots-fecha-label"></span></div>
      ...
      <div class="paso" id="paso-3">
        <div class="reserva-form">
          ...
          <div class="form-group-kp">
            ...
            <input type="tel" id="campo-telefono" placeholder="+56 9 1234 5678">
          </div>
          <div class="form-group-kp">
      ...
      <div class="paso" id="paso-confirmacion">
        <div class="confirmacion-box">
          ...
          <br><br>
          <a href="index.php" style="font-size:0.8rem; color:#888; text-decoration:none; letter-spacing:1px;">&#8592; Volver al inicio</a>
        </div>
      </div>
    </div>
  </section>
    ...
    <div class="container">
      <div class="row row-30 justify-content-center">
        <div class="col-md-4 col-sm-6">
          <a class="brand" href="index.php"><img src="images/logo-klara.png" alt="Klara Pilates" style="height:100px;width:auto"/></a>
          <ul>
...
<script>
var estado = { fechaSeleccionada: '', horaSeleccionada: '', horaFinSeleccionada: '' };
...
function volverPaso(n) { irAPaso(n); }
...
function verHorarios() {
  ...
  if (!fecha) { mostrarError('error-fecha', 'Por favor selecciona una fecha.'); return; }
  var hoy = new Date();
  hoy.setHours(0,0,0,0);
  var selDate = new Date(fecha + 'T00:00:00');
  if (selDate < hoy) { mostrarError('error-fecha', 'La fecha no puede ser anterior a hoy.'); return; }
...
function cargarSlots(fecha) {
  ...
  fetch('reservar.php', { method: 'POST', body: formData })
    ...
    .then(function(data) {
      ...
      if (data.error) { mostrarError('error-slots', data.error); return; }
...
function renderSlots(slots) {
  ...
  slots.forEach(function(slot) {
    ...
    div.innerHTML = '<span class="slot-hora">' + slot.hora + '</span>' +
      '<span class="slot-cupos ' + (slot.disponibles > 0 ? 'disponible' : 'lleno') + '">' +
      (slot.disponibles > 0 ? slot.disponibles + ' lugar' + (slot.disponibles > 1 ? 'es' : '') : 'Sin cupos') + '</span>';
    if (slot.disponibles > 0) { div.onclick = function() { seleccionarSlot(div, slot); }; }
...
function confirmarReserva() {
  ...
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { mostrarError('error-formulario', 'Ingresa un email válido.'); return; }
  ...
  fetch('reservar.php', { method: 'POST', body: formData })
    ...
    .then(function(data) {
      ...
      if (data.success) { mostrarConfirmacion(data.reserva); }
      else { mostrarError('error-formulario', data.error || 'Ocurrió un error. Intenta de nuevo.'); }
    })
    .catch(function() {
      btn.disabled = false;
      btn.textContent = 'Confirmar reserva';
